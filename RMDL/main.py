...
######################## DONOTCHANGE ###########################
def bind_model(model1, model2, model3):
    def save(dir_name):
        os.makedirs(dir_name, exist_ok=True)
        model1.save_weights(os.path.join(dir_name, 'model'))
        model2.save_weights(os.path.join(dir_name, 'model'))
        model3.save_weights(os.path.join(dir_name, 'model'))
        print('model saved!')

    def load(dir_name):
        model1.load_weights(os.path.join(dir_name, 'model'))
        model2.load_weights(os.path.join(dir_name, 'model'))
        model3.load_weights(os.path.join(dir_name, 'model'))
        print('model loaded!')

    def infer(image_path):
        result = []
        X = PathDataset(image_path, labels=None, batch_size=batch_size)
        m1_predict = model1.predict(X)
        m2_predict = model2.predict(X)
        m3_predict = model3.predict(X)
        y_hat = m1_predict + m2_predict + m3_predict
...
if __name__ == '__main__':
    ...
    DN, tDN, RN, tRN, CN, tCN = setup_nn.RMDL_model(shape=(512, 512, 3))
    DN.summary()
    RN.summary()
    CN.summary()

    # Loss and optimizer
    tDN.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    tRN.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    tCN.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

    ############ DONOTCHANGE ###############
    bind_model(tDN, tRN, tCN)
    ...
    if config.mode == 'train':  ### training mode 일때는 여기만 접근
        ...
        for epoch in range(num_epochs):
            ...
            history1 = DN.fit(train, validation_data=val, verbose=1, callbacks=[Callbacks.lr_scheduler])
            history2 = RN.fit(train, validation_data=val, verbose=1, callbacks=[Callbacks.lr_scheduler])
            history3 = CN.fit(train, validation_data=val, verbose=1, callbacks=[Callbacks.lr_scheduler])
